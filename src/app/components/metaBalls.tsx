...
void main(){
  ...
  // smooth edge
  float f = smoothstep(-1.0, 1.0, (total - 1.3) / max(1e-5, fwidth(total)));
...
void main(){
  ...
  float fw = fwidth(total);
  float f = smoothstep(-1.0, 1.0, (total - 1.3) / max(1e-5, fw));
...
const MetaBalls: React.FC<MetaBallsProps> = ({
  ...
  useEffect(() => {
    ...
    const gl: WebGLRenderingContext | WebGL2RenderingContext = renderer.gl;
    const isWebGL2 =
      (gl as WebGL2RenderingContext).TEXTURE_BINDING_3D !== undefined;
    ...
    const camera = new Camera(gl as OGLRenderingContext, {
      left: -1,
      right: 1,
      top: 1,
      bottom: -1,
    ...
    const program = new Program(gl as OGLRenderingContext, {
      vertex: isWebGL2 ? v300 : v100,
      fragment: isWebGL2 ? f300 : f100,
      uniforms: {
        iTime: { value: 0 },
    ...
    const resize = () => {
      ...
      (program.uniforms.iResolution.value as Vec3).set(
        (gl as WebGLRenderingContext).canvas.width,
        (gl as WebGLRenderingContext).canvas.height,
        0
      );
    };
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      pointerX = (px / rect.width) * (gl as WebGLRenderingContext).canvas.width;
      pointerY =
        (1 - py / rect.height) * (gl as WebGLRenderingContext).canvas.height;
    }
    ...
    const update = (t: number) => {
      ...
      if (pointerInside) {
        ...
        const cx = (gl as WebGLRenderingContext).canvas.width * 0.5;
        const cy = (gl as WebGLRenderingContext).canvas.height * 0.5;
        const rx = (gl as WebGLRenderingContext).canvas.width * 0.15;
        const ry = (gl as WebGLRenderingContext).canvas.height * 0.15;
        targetX = cx + Math.cos(elapsed * speed) * rx;
        targetY = cy + Math.sin(elapsed * speed) * ry;
      }
      mouseBallPos.x += (targetX - mouseBallPos.x) * hoverSmoothness;
      mouseBallPos.y += (targetY - mouseBallPos.y) * hoverSmoothness;
      (program.uniforms.iMouse.value as Vec3).set(
        mouseBallPos.x,
        mouseBallPos.y,
        0
      );
